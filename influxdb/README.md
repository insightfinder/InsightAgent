# influxdb
This agent collects data from influxdb and sends it to Insightfinder.
## Installing the Agent

### Short Version
```bash
bash <(curl -sS https://raw.githubusercontent.com/insightfinder/InsightAgent/master/utils/fetch-agent.sh) influxdb && cd influxdb
vi config.ini
...
```bash
curl -fsSLO https://github.com/insightfinder/InsightAgent/raw/master/influxdb/influxdb.tar.gz
tar xvf influxdb.tar.gz && cd influxdb
```
...
2to3 -w getmessages_influxdb.py; \
...
###### Test the agent:
```bash
python getmessages_influxdb.py -t
```
