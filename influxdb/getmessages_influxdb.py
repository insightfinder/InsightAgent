...
def get_agent_config_vars():
    ...
    if os.path.exists(config_ini):
        ...
        try:
            ...
            influxdb_config = {
                # connection settings
                'database': config_parser.get('influxdb', 'database'),
                'port': int(config_parser.get('influxdb', 'port')) if config_parser.get('influxdb', 'port') else 8086,
                'pool_size': int(config_parser.get('influxdb', 'pool_size')) if config_parser.get('influxdb',
                                                                                                  'pool_size') else None,
                'ssl': config_parser.get('influxdb', 'ssl').upper() == 'TRUE',
                'verify_ssl': config_parser.get('influxdb', 'verify_ssl').upper() == 'TRUE',
                'timeout': config_parser.get('influxdb', 'timeout'),
                'retries': int(config_parser.get('influxdb', 'retries')) if config_parser.get('influxdb',
                                                                                              'retries') else None,
                'use_udp': config_parser.get('influxdb', 'use_udp').upper() == 'TRUE',
                'udp_port': int(config_parser.get('influxdb', 'udp_port')) if config_parser.get('influxdb',
                                                                                                'udp_port') else None,
                'proxies': config_parser.get('influxdb', 'proxies'),
                'path': config_parser.get('influxdb', 'path'),
                'cert': config_parser.get('influxdb', 'cert'),
                'gzip': config_parser.get('influxdb', 'gzip').upper() == 'TRUE',
                'session': config_parser.get('influxdb', 'session'),
            }
            # only keep settings with values
            influxdb_kwargs = {k: v for (k, v) in influxdb_config.items() if v}

            # handle required arrays
            # host
            if len(config_parser.get('influxdb', 'host')) != 0:
                influxdb_kwargs['host'] = config_parser.get('influxdb', 'host')
            else:
                config_error('host')
            if len(config_parser.get('influxdb', 'username')) != 0:
                influxdb_kwargs['username'] = config_parser.get('influxdb', 'username')
            else:
                config_error('user')
            if len(config_parser.get('influxdb', 'password')) != 0:
                influxdb_kwargs['password'] = config_parser.get('influxdb', 'password')
            else:
            ...
            if len(config_parser.get('influxdb', 'sql')) != 0:
                ...
                config_error('sql')
            if len(config_parser.get('influxdb', 'sql_time_format')) != 0:
            ...
            if len(config_parser.get('influxdb', 'sql_time_range')) != 0 and len(
                    config_parser.get('influxdb', 'sql_time_interval')) != 0:
                try:
                    sql_time_range = filter(lambda x: x.strip(),
                                            config_parser.get('influxdb', 'sql_time_range').split(','))
                    sql_time_range = map(lambda x: int(arrow.get(x).float_timestamp), sql_time_range)
                    sql_time_interval = int(config_parser.get('influxdb', 'sql_time_interval'))
                    sql_config = {
            ...
            # proxies
            agent_http_proxy = config_parser.get('influxdb', 'agent_http_proxy')
            agent_https_proxy = config_parser.get('influxdb', 'agent_https_proxy')

            # metric buffer
            all_metrics = config_parser.get('influxdb', 'all_metrics')
            metric_buffer_size_mb = config_parser.get('influxdb', 'metric_buffer_size_mb') or '10'

            # filters
            filters_include = config_parser.get('influxdb', 'filters_include')
            filters_exclude = config_parser.get('influxdb', 'filters_exclude')

            # message parsing
            data_format = config_parser.get('influxdb', 'data_format').upper()
            raw_regex = config_parser.get('influxdb', 'raw_regex', raw=True)
            raw_start_regex = config_parser.get('influxdb', 'raw_start_regex', raw=True)
            csv_field_names = config_parser.get('influxdb', 'csv_field_names')
            csv_field_delimiter = config_parser.get('influxdb', 'csv_field_delimiter', raw=True) or CSV_DELIM
            json_top_level = config_parser.get('influxdb', 'json_top_level')
            # project_field = config_parser.get('influxdb', 'project_field', raw=True)
            instance_field = config_parser.get('influxdb', 'instance_field', raw=True)
            device_field = config_parser.get('influxdb', 'device_field', raw=True)
            timestamp_field = config_parser.get('influxdb', 'timestamp_field', raw=True) or 'timestamp'
            timestamp_format = config_parser.get('influxdb', 'timestamp_format', raw=True)
            timezone = config_parser.get('influxdb', 'timezone')
            data_fields = config_parser.get('influxdb', 'data_fields', raw=True)
