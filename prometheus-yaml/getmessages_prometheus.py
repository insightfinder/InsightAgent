...
def initialize_cache_connection():
    ...
    if os.path.exists(cache_loc):
        ...
        cache_cur = cache_con.cursor()
        cache_cur.execute(
            'CREATE TABLE "cache" ( "instance"	TEXT NOT NULL UNIQUE, "alias"	TEXT NOT NULL)'
        )
...
def start_data_processing(
    ...
    def run_prometheus_query(timestamp):
        ...
        for query in prometheus_query:
            ...
            if metric_batch_size:
                all_metric_list = query_prometheus_labels(
                ...
                if batch_metric_filter_regex:
                    metric_list = [
                        m
                        for m in all_metric_list
                        if regex.match(batch_metric_filter_regex, m)
                    ]
                logger.info(
                    "Metrics found {} and filtered {}: {}".format(
                        len(all_metric_list), len(metric_list), metric_list
                    )
                )
                if len(metric_list) > 0:
                    metric_batch_list = [
                        metric_list[i : i + metric_batch_size]
                        for i in range(0, len(metric_list), metric_batch_size)
                    ]
                else:
            ...
            for metric_batch in metric_batch_list:
                ...
                if len(metric_batch) > 0:
                    mql["query"] = " or ".join(
                        ["{}{}".format(m, query_str) for m in metric_batch]
                    )
                else:
            ...
            params = [
                (logger, if_config_vars, agent_config_vars, None, mql)
                for mql in query_list
            ]
            results = thread_pool.map(query_messages_prometheus, params)
            result_list = list(chain(*results))
            parse_messages_prometheus(
                logger,
                if_config_vars,
                agent_config_vars,
                metric_buffer,
    ...
    if agent_config_vars["his_time_range"]:
        logger.debug(
            "history range config: {}".format(agent_config_vars["his_time_range"])
        )
        for timestamp in range(
            agent_config_vars["his_time_range"][0],
...
def query_prometheus_labels(args):
    ...
    try:
        ...
        url = urllib.parse.urljoin(
            agent_config_vars["api_url"], "label/__name__/values"
        )
        response = send_request(
            logger,
            url,
            proxies=agent_config_vars["proxies"],
            **agent_config_vars["auth_kwargs"],
...
def parse_messages_prometheus(
    ...
    sampling_time = (
        sampling_time * 1000 if len(str(sampling_time)) == 10 else sampling_time
    )
        ...
        try:
            ...
            if query_metric_name:
                ...
                if metrics_name_field:
                    name_fields = [
                        message.get("metric").get(f) for f in metrics_name_field or []
                    ]
            ...
            if instance_field and len(instance_field) > 0:
                ...
                instance = (
                    agent_config_vars["instance_connector"].join(instance_list)
            ...
            if device_field and len(device_field) > 0:
                ...
                device = (
                    agent_config_vars["instance_connector"].join(devices)
                    if len(devices) > 0
                    else None
                )
            ...
            if agent_config_vars["component_field"]:
                component = message.get("metric").get(
                    agent_config_vars["component_field"]
                )
            ...
            if agent_config_vars["dynamic_host_field"]:
                host_id = message.get("metric").get(
                    agent_config_vars["dynamic_host_field"]
                )

            vector_value = message.get("value")
            timestamp = (
                int(vector_value[0]) * 1000
                if len(str(vector_value[0])) == 10
                else vector_value[0]
            )
            if sampling_time and abs(timestamp - sampling_time) > sampling_interval:
                timestamp = sampling_time
                logger.warn(
                    "Timestamp %s in the message is not in the sampling interval, using sampling time %s"
                    % (timestamp, sampling_time)
                )
    ...
    try:
        if cache_cur:
            ...
            if instance:
                ...
                cache_cur.execute(
                    'insert into cache (instance, alias) values ("%s", "%s")'
                    % (alias, alias)
                )
                cache_con.commit()
                return alias
...
def get_agent_config_vars(logger, config_yaml, project_name):
    ...
    try:
        ...
        prometheus_config = {
            ...
            "verify_certs": config.prometheus.verify_certs,
            "ca_certs": config.prometheus.ca_certs,
            "client_cert": config.prometheus.client_cert,
            "client_key": config.prometheus.client_key,
        }
        ...
        ssl_kwargs = {
            "verify": verify_certs,
            "cert": cert if verify_certs else None,
        }

        # handle required arrays
        if config.prometheus.prometheus_uri:
            prometheus_uri = config.prometheus.prometheus_uri
            api_url = urllib.parse.urljoin(prometheus_uri, "api/v1/")
        else:
            return config_error(logger, "prometheus_uri")

        # metrics
        metrics_name_field = project.metrics_name_field
        prometheus_query_str = project.prometheus_query
        prometheus_query_json = project.prometheus_query_json

        prometheus_query_metric_batch_size = project.prometheus_query_metric_batch_size
        batch_metric_filter_regex = project.batch_metric_filter_regex
        # time range
        his_time_range = project.his_time_range

        # proxies
        agent_http_proxy = config.prometheus.agent_http_proxy
        agent_https_proxy = config.prometheus.agent_https_proxy

        # message parsing
        data_format = project.data_format.upper()
        # project_field = config_parser.get('agent', 'project_field', raw=True)
        component_field = project.component_field
        dynamic_host_field = project.dynamic_host_field
        default_component_name = project.default_component_name
        instance_field = project.instance_field
        instance_whitelist = project.instance_whitelist
        device_field = project.device_field
        timestamp_field = project.timestamp_field or "timestamp"
        target_timestamp_timezone = project.target_timestamp_timezone or "UTC"
        timestamp_format = project.timestamp_format
        timezone = project.timezone or "UTC"
        instance_connector = project.instance_connector or "-"
        thread_pool = config.prometheus.thread_pool
        processes = config.prometheus.processes
        timeout = config.prometheus.timeout
        instance_name_suffix = project.instance_name_suffix or ""
    ...
    prometheus_query = []
    if prometheus_query_str is not None and (len(prometheus_query_str) != 0):
        ...
        for query in queries:
            if ":" in query:
                ...
                instance_fields = (
                    [x.strip() for x in parts[1].split(",") if x.strip()]
                    if parts[1]
                    else []
                )
                qml = {
    ...
    if prometheus_query_json:
        try:
            json_path = os.path.join(
                os.path.dirname(config_yaml), prometheus_query_json
            )
    ...
    # metrics
    if instance_whitelist is not None and (len(instance_whitelist) != 0):
        try:
    ...
    if metrics_name_field is not None and (len(metrics_name_field) != 0):
        metrics_name_field = [
            x.strip() for x in metrics_name_field.split(",") if x.strip()
        ]

    if his_time_range is not None and (len(his_time_range) != 0):
    ...
    if len(target_timestamp_timezone) != 0:
        target_timestamp_timezone = int(
            arrow.now(target_timestamp_timezone).utcoffset().total_seconds()
        )
    else:
    ...
    if thread_pool and len(thread_pool) != 0:
    ...
    # proxies
    agent_proxies = dict()
    if agent_http_proxy is not None and (len(agent_http_proxy) > 0):
        agent_proxies["http"] = agent_http_proxy
    if agent_https_proxy is not None and (len(agent_https_proxy) > 0):
    ...
    config_vars = {
        ...
        "dynamic_host_field": dynamic_host_field,
...
def get_cli_config_vars():
    ...
    parser.add_option(
        ...
        action="store",
        dest="config",
        default=abs_path_from_cur("conf.d/config.yaml"),
        help="Path to the config files to use. Defaults to {}".format(
            abs_path_from_cur("conf.d/config.yaml")
        ),
    )
    ...
    config_vars = {
        ...
        else abs_path_from_cur("conf.d/config.yaml"),
        # 'processes': int(options.processes),
        "testing": False,
        "log_level": logging.INFO,  # 'timeout': int(options.timeout) * 60,
    }
...
def get_if_config_vars(logger, config_yaml, project_name):
    ...
    try:
        ...
        chunk_size_kb = config.insightfinder.chunk_size_kb
        if_url = config.insightfinder.if_url
        if_http_proxy = config.insightfinder.if_http_proxy
        if_https_proxy = config.insightfinder.if_https_proxy
        dynamic_metric_type = project.dynamic_metric_type
    ...
    if license_key is not None and (len(license_key) == 0):
    ...
    if project_type is not None and (len(project_type) == 0):
    ...
    if sampling_interval is not None and (len(sampling_interval) == 0):
    ...
    if run_interval is not None and (len(run_interval) == 0):
    ...
    # defaults
    if chunk_size_kb is not None and (len(chunk_size_kb) == 0):
        chunk_size_kb = 2048  # 2MB chunks by default
    if if_url is not None and (len(if_url) == 0):
        if_url = "https://app.insightfinder.com"

    # set IF proxies
    if_proxies = dict()
    if if_http_proxy is not None and (len(if_http_proxy) > 0):
        if_proxies["http"] = if_http_proxy
    if if_https_proxy is not None and (len(if_https_proxy) > 0):
...
def initialize_data_gathering(
    logger, c_config, if_config_vars, agent_config_vars, time_now
):
    metric_buffer = dict()
    track = dict()
...
def clear_metric_buffer(logger, c_config, if_config_vars, metric_buffer, track):
    ...
    for row in buffer_values:
        ...
        if (
            count % 100 == 0
            or get_json_size_bytes(track["current_row"]) >= if_config_vars["chunk_size"]
        ):
...
def send_data_wrapper(logger, c_config, if_config_vars, track):
    ...
    logger.debug(
        "--- Chunk creation time: {} seconds ---".format(
...
def convert_to_metric_data(logger, chunk_metric_data, cli_config_vars, if_config_vars):
    ...
    for chunk in chunk_metric_data:
        ...
        if data and timestamp and instance_name:
            ...
            if host_id:
                instance_data_map[instance_name]["dit"][timestamp]["k"] = {
                    "hostId": host_id
                }
...
def send_data_to_if(logger, cli_config_vars, if_config_vars, track, chunk_metric_data):
    ...
    if "METRIC" in if_config_vars["project_type"]:
        json_to_post = convert_to_metric_data(
            logger, chunk_metric_data, cli_config_vars, if_config_vars
        )
        logger.debug(json_to_post)
        post_url = urllib.parse.urljoin(
            if_config_vars["if_url"], "api/v2/metric-data-receive"
        )
    else:
        data_to_post = initialize_api_post_data(logger, if_config_vars)
        if (
            "DEPLOYMENT" in if_config_vars["project_type"]
            or "INCIDENT" in if_config_vars["project_type"]
        ):
            for chunk in chunk_metric_data:
                chunk["data"] = json.dumps(chunk["data"])
        data_to_post[get_data_field_from_project_type(if_config_vars)] = json.dumps(
            chunk_metric_data
        )
        # add component mapping to the post data
        track["component_map_list"] = list(
            {v["instanceName"]: v for v in track["component_map_list"]}.values()
        )
        data_to_post["instanceMetaData"] = json.dumps(track["component_map_list"] or [])

        post_url = urllib.parse.urljoin(
            if_config_vars["if_url"], get_api_from_project_type(if_config_vars)
        )
    ...
    if data_to_post:
        ...
        send_request(
            ...
            "POST",
            "Could not send request to IF",
            str(get_json_size_bytes(json_to_post)) + " bytes of data are reported.",
            json=json_to_post,
            verify=False,
            proxies=if_config_vars["if_proxies"],
        )
    ...
    logger.info(
        "--- Send data time: %s seconds ---" % round(time.time() - send_data_time, 2)
    )
...
def initialize_api_post_data(logger, if_config_vars):
    ...
    if (
        "METRIC" in if_config_vars["project_type"]
        and "sampling_interval" in if_config_vars
    ):
...
def check_project_exist(logger, if_config_vars):
    ...
    try:
        ...
        url = urllib.parse.urljoin(
            if_config_vars["if_url"], "api/v1/check-and-add-custom-project"
        )
        response = send_request(
            logger,
            url,
    ...
    if not is_project_exist:
        try:
            ...
            params = {
                ...
                "licenseKey": if_config_vars["license_key"],
                "projectName": if_config_vars["project_name"],
                "systemName": if_config_vars["system_name"]
                or if_config_vars["project_name"],
            ...
            logger.debug(
                "insightAgentType:",
                get_insight_agent_type_from_project_type(if_config_vars),
            )
            url = urllib.parse.urljoin(
                if_config_vars["if_url"], "api/v1/check-and-add-custom-project"
            )
            ...
            if response == -1:
                ...
                if result["success"] is False:
                    ...
                    logger.info(
                        "Add project success: " + if_config_vars["project_name"]
                    )
    ...
    if create_project_sucess:
        ...
        try:
            ...
            if response == -1:
                ...
                if result["success"] is False or result["isProjectExist"] is False:
                    logger.error(
                        "Check project error: " + if_config_vars["project_name"]
                    )
                else:
                    is_project_exist = True
                    logger.info(
                        "Check project success: " + if_config_vars["project_name"]
                    )
