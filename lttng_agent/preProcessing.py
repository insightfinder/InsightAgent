...
#Takes a list of system calls and splits the list based on large time gaps and TID.
def outputLog(systemCallList,outFileName,procName):
    ...
    preTID = -1
    preContextTID = -1
    pNamesSplit = procName.split(",")
    procOut = pNamesSplit[0]
            ...
            for item in syscallList:                        
                if pTime != -1:
                    ...
                    if diff >= intervals[item.tid]:
                        ...
                        outputList(currentList,outFile,procOut)
                        currentList = []
...
#Outputs the current list to the output file
...
#Parses the current line and puts the results into the system call data structure format
...
#Segments the input file based on process name while inserting cotext switch markers into the list. Hands resulting list to the outputLog function to be further segmented by TID and large time gaps.
