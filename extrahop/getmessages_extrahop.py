...
def start_data_processing(logger, c_config, if_config_vars, agent_config_vars, metric_buffer, track, time_now):
    ...
    if device_ip_list:
        ...
        def query_devices(args):
            ...
            try:
                # execute sql string
                response = send_request(logger, url, headers=headers, params=params, verify=False,
...
def query_messages_extrahop(args):
    ...
    try:
        ...
        response = send_request(logger, url, mode='POST', headers=headers, data=json.dumps(params), verify=False,
                                proxies=agent_config_vars['proxies'])
        if response == -1:
