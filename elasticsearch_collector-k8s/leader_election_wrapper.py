#!/usr/bin/env python3
"""
Leader Election Wrapper for elasticsearch_collector Agent

This wrapper implements Kubernetes leader election to ensure only one replica
of the elasticsearch_collector agent collects data, preventing duplicates when scaling.

The wrapper runs the original getmessages_elasticsearch_collector.py script only when
...
class LeaderElectionWrapper:
    def __init__(self):
        ...
        self.pod_name = os.getenv("HOSTNAME", "elasticsearch-collector-unknown")
        self.lease_name = os.getenv("LEASE_NAME", "elasticsearch-collector-leader")
    ...
    def _stop_subprocess(self):
        ...
        if self.subprocess and self.subprocess.poll() is None:
            logger.info("Stopping elasticsearch_collector agent subprocess...")
    ...
    def _run_elasticsearch_collector_agent(self):
        """Run the original elasticsearch_collector agent script (cron.py)"""
        script_path = os.path.join(os.path.dirname(__file__), "cron.py")
        cmd = [sys.executable, script_path] + sys.argv[
            1:
        ]  # Pass through any command line arguments

        logger.info(f"Starting elasticsearch_collector agent: {' '.join(cmd)}")
        ...
        try:
            ...
            while True:
                ...
                if not line:
                    break
                logger.info(f"[elasticsearch-collector] {line.strip()}")
                if self.should_stop:
                    break
            ...
            if return_code != 0 and not self.should_stop:
                logger.error(f"elasticsearch_collector agent exited with code {return_code}")
            else:
                logger.info("elasticsearch_collector agent completed successfully")

        except Exception as e:
            logger.error(f"Failed to run elasticsearch_collector agent: {e}")
        finally:
            self.subprocess = None
        ...
        while not self.should_stop:
            try:
                ...
                if acquired_leadership and not self.is_leader:
                    ...
                    logger.info("🎉 Acquired leadership! Starting elasticsearch_collector agent...")
                    self.is_leader = True

                    # Start the elasticsearch_collector agent in a separate thread
                    self.agent_thread = threading.Thread(
                        target=self._run_elasticsearch_collector_agent
                    )
                    ...
                    # We lost leadership
                    logger.info("❌ Lost leadership! Stopping elasticsearch_collector agent...")
                    self.is_leader = False
                    self._stop_subprocess()
                    ...
                    if self.subprocess and self.subprocess.poll() is not None:
                        logger.info(
                            "elasticsearch_collector agent process ended, will restart on next cycle"
                        )
                        self.subprocess = None
