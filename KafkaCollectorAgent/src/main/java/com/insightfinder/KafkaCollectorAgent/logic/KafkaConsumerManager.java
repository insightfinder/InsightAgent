...
public class KafkaConsumerManager {
  ...
  public void init() {
    ...
    for (Map<String, String> cluster : clusterInfo.values()) {
      ...
      customizers.orderedStream().forEach(
          customizer -> customizer.customize((DefaultKafkaConsumerFactory<?, ?>) consumerFactory));
      ConcurrentKafkaListenerContainerFactory<String, String> factory = new ConcurrentKafkaListenerContainerFactory<>();
      factory.setBatchListener(true);
      factory.setConsumerFactory(consumerFactory);
      IFKafkaListenerEndpoint ifKafkaListenerEndpoint = new IFKafkaListenerEndpoint(
          cluster.get("topic"), cluster.get("group.id"),
          Integer.valueOf(cluster.get("concurrency")));
      ConcurrentMessageListenerContainer<String, String> container = factory.createListenerContainer(
          ifKafkaListenerEndpoint);
      if (ifConfig.isFastRecovery()) {
        ...
        container.setupMessageListener((MessageListener<Integer, String>) record -> {
          ifStreamingBufferManager.parseString(record.topic(), record.value(),
              System.currentTimeMillis());
        });
      }
      applicationContext.registerBeanDefinition("ConcurrentMessageListenerContainer" + clusterIndex,
          BeanDefinitionBuilder.genericBeanDefinition(ConcurrentMessageListenerContainer.class,
              () -> container).getBeanDefinition());
      clusterIndex++;
    }
  }
  ...
  public static class IFMessageListener implements MessageListener<Integer, String>,
      ConsumerSeekAware {
