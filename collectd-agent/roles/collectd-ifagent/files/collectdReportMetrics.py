...
def aggregate_results_into_raw_data(start_time_epoch_l, new_prev_endtime_epoch_l, date_l):
    ...
    filenames = {'cpu/percent-active-': ['CPU'], 'memory/memory-used-': ['MemUsed'],
                 'load/load-': ['LoadAvg1', 'LoadAvg5', 'LoadAvg15'],
...
def remove_old_files_and_update_filesnames(filenames):
    ...
    for each_dir in all_directories:
        ...
        if "df-" in each_dir:
            filenames[each_dir + "/df_complex-used-"] = [each_dir[3:] + '_DiskUsed']
            filenames[each_dir + "/df_complex-free-"] = [each_dir[3:] + '_DiskFree']
...
def set_raw_data_from_collectd_dir(aggregate_cpu, all_latest_timestamps, date_l, filenames, new_prev_endtime_epoch_l,
    ...
    for each_file in filenames:
        if "cpu/percent-active" in each_file and aggregate_cpu:
            continue
        if "df-" in each_file:
            new_prev_endtime_epoch_l = calculate_df_disk_used_free_bytes(all_latest_timestamps, each_file, filenames,
                                                                new_prev_endtime_epoch_l, raw_data_l,
                                                                start_time_epoch_l, date_l)
        elif "aggregation-cpu-average/cpu-system" in each_file and aggregate_cpu:
...
def fill_metric_data_to_send(raw_data_l, hostname_short_l):
    ...
    for each_timestamp in raw_data_l:
        ...
        # this_data['timestamp'] = str(int(each_timestamp) * 1000)
        disk_read = disk_write = network_in = network_out = disk_used = disk_total = 0  # diskused

        new_result, this_data = get_new_object_from_disk_and_network_details(data, delta_fields, disk_read, disk_write, disk_used, disk_total,
...
def get_new_object_from_disk_and_network_details(data, delta_fields, disk_read, disk_write, disk_used, disk_total, hostname_short_l,
                                                 metric_list, network_in, network_out,
                                                 previous_result_l, this_data):
    new_result = {}
    for each_metric in metric_list:
        if each_metric == "DiskWrite" or each_metric == "DiskRead" \
                or each_metric == "NetworkIn" or each_metric == "NetworkOut" or each_metric == "DiskUsed":
        ...
        if (not is_str_in_keys(data, each_metric)) and each_metric != "DiskRead" and each_metric != "DiskWrite" \
            ...
            elif each_metric == "DiskUsed":
                if disk_total != 0:
                    this_data[final_metric_name] = str(float(float(disk_used/disk_total) * 100))
                else:
                    this_data[final_metric_name] = "0"
            else:
