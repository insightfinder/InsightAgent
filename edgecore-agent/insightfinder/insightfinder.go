...
func NewService(config config.InsightFinderConfig) *Service {
	...
	service := &Service{
		...
		LogProjectName: config.LogsProjectName,    // Separate logs project
...
func (s *Service) updateDerivedTypes() {
	...
	case "Metric", "METRIC":
		...
		if s.Container {
			...
			s.InsightAgentType = "PositronAgent"
		}
	default:
		s.DataType = pt
		if s.Container {
...
func (s *Service) SendMetricsBulk(metrics []models.MetricData) error {
	...
	logrus.Debugf("Sending %d metrics in bulk to InsightFinder using v2 API", len(metrics))
