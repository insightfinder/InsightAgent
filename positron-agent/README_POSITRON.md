...
The component name sent to InsightFinder is fixed to `Positron Agent` for every instance (endpoints and devices alike) - it is not configurable.
...
- **Component name** (`cn`): fixed to `Positron Agent` for every instance (endpoints and devices alike) - not from Inventory or config.
- **Zone** (`z`): not currently populated - left empty for now, pending a redesign of how it should be sourced.
