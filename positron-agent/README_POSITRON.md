...
- **Component name** (`cn`): fixed to `Positron Agent` for every instance (endpoints and devices alike).
- **Zone** (`z`) / subvenue: temporarily disabled - always empty for now, regardless of what Inventory has. Will be reinstated separately.
