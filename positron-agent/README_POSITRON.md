...
- **Component name** (`cn`): not from Inventory - from config (`positron.endpoint_component_name` default `Positron-Endpoint`, `positron.gam_component_name` default `Positron-GAM`), picked per record rather than fixed per API source. `/device/list` and `/endpoint/list/all` each mix in both GN (regular CPE) and GAM (headend) units, so the pick is: endpoints use `ModelType` (`"GAM-C"` -> GAM, anything else -> endpoint), devices use `ProductClass` (contains `"GAM"` -> GAM, else endpoint); when that field is empty (endpoint offline, model undetected) fall back to checking the record's own name for a `"GAM"`/`"GN"` marker, and default to the record's natural source (endpoint/GAM respectively) if even that finds nothing.
