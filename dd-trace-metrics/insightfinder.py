...
def check_and_create_project(
    *,
    ...
    """
    Check whether the InsightFinder project exists and create it if not.
    Returns True if the project is ready to receive data, False otherwise.

    Mirrors the behaviour of check_project_exist() in the prometheus agent.
    """
...
def send_metric_data_to_if(
    *,
    ...
    """Send metric rows to InsightFinder.

    Each row in *metric_rows* must contain:
        instanceName  (str)  – host / service identifier
        timestamp     (int)  – epoch milliseconds
    Any additional keys are treated as metric name → float value pairs.
