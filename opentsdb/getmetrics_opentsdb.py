...
def getAgentConfigVars():
    configVars = {}
    try:
        if os.path.exists(os.path.join(parameters['homepath'], "opentsdb", "config.ini")):
            parser = SafeConfigParser()
            parser.read(os.path.join(parameters['homepath'], "opentsdb", "config.ini"))
            insightFinder_license_key = parser.get('opentsdb', 'insightFinder_license_key')
            insightFinder_project_name = parser.get('opentsdb', 'insightFinder_project_name')
            insightFinder_user_name = parser.get('opentsdb', 'insightFinder_user_name')
            sampling_interval = parser.get('opentsdb', 'sampling_interval')
