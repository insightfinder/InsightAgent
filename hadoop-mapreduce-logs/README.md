...
This agent collects data from hadoop-mapreduce-logs and sends it to Insightfinder.
...
2to3 -w getlogs_hadoop-mapreduce.py; \
...
### Config Variables
* **`history_server_uri`**: History server URI, as `<scheme>://server:port`.
* `user`: User to find jobs for.
* `queue`: Queue to find jobs in.
