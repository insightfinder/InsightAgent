...
def start_data_processing(thread_number):
    set_time_range()
    logger.debug(str(agent_config_vars['parameters']))
    jobs = get_job_list()
    for job in jobs:
        get_job_data(str(job['id']))
...
def get_agent_config_vars():
    ...
    if os.path.exists(os.path.abspath(os.path.join(__file__, os.pardir, 'config.ini'))):
        ...
        try:
            ...
            queue = config_parser.get('agent', 'queue')
            ...
            # message parsing
            json_top_level = config_parser.get('agent', 'json_top_level') or 'job'
            #project_field = config_parser.get('agent', 'project_field') # not yet ready. need to group messages/metrics by project
            instance_field = config_parser.get('agent', 'instance_field') or 'queue'
            device_field = config_parser.get('agent', 'device_field')
            timestamp_field = config_parser.get('agent', 'timestamp_field') or 'finishTime'
