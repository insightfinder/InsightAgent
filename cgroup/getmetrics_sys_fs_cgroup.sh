...
for container in $dockers; do
    ...
    (cat /sys/fs/cgroup/memory/docker/$CONTAINER_ID/memory.usage_in_bytes | awk '{print "MemUsed="$1}' > memmetrics_$CONTAINER_NAME.txt ;
    cat /sys/fs/cgroup/memory/docker/$CONTAINER_ID/memory.limit_in_bytes | awk 'BEGIN{mem_total=0} {if($1<=999999999){mem_total+=$1}} END{print "MemTotal="mem_total}' >> memmetrics_$CONTAINER_NAME.txt ;
    ...
    cat /sys/fs/cgroup/blkio/docker/$CONTAINER_ID/blkio.throttle.io_service_bytes | grep Read | awk 'BEGIN{readbytes=0} {if(NR==1){readbytes+=$3}} END{print "DiskRead="readbytes}' > diskmetricsread_$CONTAINER_NAME.txt & PID3=$!
    cat /sys/fs/cgroup/blkio/docker/$CONTAINER_ID/blkio.throttle.io_service_bytes | grep Write | awk 'BEGIN{writebytes=0} {if(NR==1){writebytes+=$3;}} END{print "DiskWrite="writebytes}' > diskmetricswrite_$CONTAINER_NAME.txt & PID4=$!
    ...
    docker exec $CONTAINER_ID df -k -x tmpfs -x devtmpfs | awk 'BEGIN{diskusedspace=0; totalspace=0}{if(NR!=1)if($3!="")print "DiskUsed"$6"="$3; diskusedspace += $3; totalspace += $4}END{print "DiskUsed="(diskusedspace/totalspace)*100}' > diskusedmetrics_$CONTAINER_NAME.txt & PID7=$!
