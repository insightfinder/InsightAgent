...
for container in $dockers; do
    ...
    cat /sys/fs/cgroup/memory/docker/$container/memory.limit_in_bytes | awk 'BEGIN{mem_total=0} {if($1<=999999999){mem_total+=$1}} END{print "MemTotal="mem_total}' >> memmetrics_$container.txt & PID3=$!
    cat /sys/fs/cgroup/blkio/docker/$container/blkio.throttle.io_service_bytes | grep Read | awk 'BEGIN{readbytes=0} {if(NR!=1){readbytes+=$3}} END{print "DiskRead="readbytes}' > diskmetricsread_$container.txt & PID4=$!
    cat /sys/fs/cgroup/blkio/docker/$container/blkio.throttle.io_service_bytes | grep Write | awk 'BEGIN{writebytes=0} {if(NR!=1){writebytes+=$3;}} END{print "DiskWrite="writebytes}' > diskmetricswrite_$container.txt & PID5=$!
    ...
    cat /sys/fs/cgroup/cpuacct/docker/$container/cpuacct.stat | awk 'BEGIN{cpu=0} {cpu+=$2} END{print "CPU="cpu}' > cpumetrics_$container.txt & PID7=$!
