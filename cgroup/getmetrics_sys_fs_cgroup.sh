...
for container in $dockers; do
    ...
    (cat /sys/fs/cgroup/memory/docker/$container/memory.usage_in_bytes | awk '{print "MemUsed="$1}' > memmetrics_$container.txt ;
    cat /sys/fs/cgroup/memory/docker/$container/memory.limit_in_bytes | awk 'BEGIN{mem_total=0} {if($1<=999999999){mem_total+=$1}} END{print "MemTotal="mem_total}' >> memmetrics_$container.txt ;
