...
for container in $dockers; do
    ...
    docker exec $CONTAINER_ID df -k -x tmpfs -x devtmpfs | awk 'BEGIN{diskusedspace=0; totalspace=0}{if(NR!=1)if($3!="")print "DiskUsed"$6"="$3; diskusedspace += $3; totalspace += $4}END{print "DiskUsed="(diskusedspace/totalspace)*100}' > diskusedmetrics$CONTAINER_NAME.txt & PID7=$!
