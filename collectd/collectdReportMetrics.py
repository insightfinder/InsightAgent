...
for eachfile in filenames:
    ...
    if "aggregation-cpu-average/cpu-system" in eachfile and aggregateCPU == True:
        try:
            ...
            csvfile2 = open(os.path.join(
                csvpath, 'aggregation-cpu-average/cpu-user-' + date))
            csvfile3 = open(os.path.join(
                csvpath, 'aggregation-cpu-average/cpu-idle-' + date))
        ...
        for row, row1, row2 in itertools.izip(reader1, reader2, reader3):
            if reader1.line_num > 1:
                ...
                if timestampStr in rawData:
                    ...
                    valueList[filenames[eachfile][0]] = str(
    ...
    for eachmetric in metricList:
        ...
        if (isStrInKeys(data, eachmetric) == False) and eachmetric != "DiskRead" and eachmetric != "DiskWrite" and eachmetric != "NetworkIn" and eachmetric != "NetworkOut":
            ...
            if eachmetric == "DiskWrite":
                ...
                thisData[finalMetricName] = str(
                    float(float(diskread) / (1024 * 1024)))
            elif eachmetric == "NetworkIn":
                thisData[finalMetricName] = str(
                    float(float(networkin) / (1024 * 1024)))
            elif eachmetric == "NetworkOut":
                thisData[finalMetricName] = str(
                    float(float(networkout) / (1024 * 1024)))
            elif eachmetric == "MemUsed":
                thisData[finalMetricName] = str(
                    float(float(data[eachmetric]) / (1024 * 1024)))
            else:
            ...
            if eachmetric in deltaFields:
                if finalMetricName in previousResult:
                    thisData[finalMetricName] = str(
                        abs(float(thisData[finalMetricName]) - float(previousResult[finalMetricName])))
                else:
                    thisData[finalMetricName] = "NaN"
...
reported = reportCustomMetrics.getcustommetrics(
    serverUrl, PROJECTNAME, USERNAME, LICENSEKEY, homepath)
if reported:
