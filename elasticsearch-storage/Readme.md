# InsightAgent: elasticsearch-storage
Agent Type: elasticsearch-storage
...
##### Pre-requisites:
Elasticsearch should be installed and accessible on port 9200 (This can be changed via the [configuration file](#config) ). The "index" name the agent reads from is ***"if_metric_csv"*** and the "type" is ***csv***.
...
./deployInsightAgent.sh -n USER_NAME_IN_HOST
                        ...
                        -t AGENT_TYPE
                        -w SERVER_URL
AGENT_TYPE is *elasticsearch-storage*.
```
