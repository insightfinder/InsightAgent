...
def batch_update_instance_component_name(project_name: str, instance_component_name_dict: dict):
    url = f"{config.insightfinder_url}/api/v1/agent-upload-instancemetadata?userName={config.insightfinder_username}&licenseKey={config.license_key}&projectName={project_name}&override=true"
...
def save_component_files(logger, instance_component_name_dict):
    """Save matched and unmatched hosts to separate JSON files"""
    try:
...
#######################    
# def save_component_files_debug(logger, matched_hosts, unmatched_hosts):
#     """Save matched and unmatched hosts to separate JSON files"""
#     try:
#         # Save matched hosts
#         with open(MATCHED_FILE, 'w') as f:
#             json.dump(matched_hosts, f, indent=2)
#         logger.info(f"Matched components saved to {MATCHED_FILE}")
        
#         # Save unmatched hosts
#         with open(UNMATCHED_FILE, 'w') as f:
#             json.dump(unmatched_hosts, f, indent=2)
#         logger.info(f"Unmatched components saved to {UNMATCHED_FILE}")

#         # Print component summary
#         logger.info(f"Component matching summary:")
#         logger.info(f"  Matched hosts: {len(matched_hosts)}")
#         logger.info(f"  Unmatched hosts: {len(unmatched_hosts)}")
        
#         # Count by component type
#         if matched_hosts:
#             component_counts = {}
#             for host in matched_hosts:
#                 comp_type = host['component_name']
#                 component_counts[comp_type] = component_counts.get(comp_type, 0) + 1
            
#             logger.info("Component type distribution:")
...
# def categorize_hosts_by_component(metadata):
#     """Categorize hosts into matched and unmatched based on component regex"""
#     matched_hosts = []
#     unmatched_hosts = []
...
#         if component_type:
#             matched_hosts.append(host_component_data)
#         else:
#             unmatched_hosts.append(host_component_data)
    
#     return matched_hosts, unmatched_hosts
#######################
...
def main():
    ...
    ################
    # # Categorize hosts by component type
    # logger.info("Categorizing hosts by component type...")
    # matched_hosts, unmatched_hosts = categorize_hosts_by_component(metadata)

    # # Save instance component mapping to a JSON file
    # logger.info("Saving instance component mapping to JSON file...")
    # if not save_component_files_debug(logger, matched_hosts, unmatched_hosts):
    #     sys.exit(1)
    ################
    ...
    # Send metadata api request
    if not batch_update_instance_component_name(config.insightfinder_project, instance_component_name_dict):
        logger.error("Failed to update instance component names via API")
        sys.exit(1)
