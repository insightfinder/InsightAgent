...
def get_app_list():
    ...
    apps = json.loads(response_raw.text)
    return apps


def get_app_data(app):
    app_api_url = urlparse.urljoin(agent_config_vars['api_url'], app['id'] + '/jobs')
    response_raw = send_request(app_api_url, proxies=agent_config_vars['proxies'])
    jobs = json.loads(response_raw.text)
    for job in jobs:
        # prevent collision
        for key in job:
            if key in app:
...
def get_agent_config_vars():
    ...
    if os.path.exists(os.path.abspath(os.path.join(__file__, os.pardir, 'config.ini'))):
        ...
        try:
            ...
            instance_field = config_parser.get('agent', 'instance_field') or 'name'
...
def send_data_to_if(chunk_metric_data):
    ...
    check_project(data_to_post['projectName'])
