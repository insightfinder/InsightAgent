...
func NewService(config config.InsightFinderConfig) *Service {
	...
	service := &Service{
		...
		ProjectType: "Metric", // Default for Cambium metrics
...
func (s *Service) Validate() bool {
	...
	if s.ProjectType == "Metric" {
		if s.Container {
			...
			s.InsightAgentType = "CambiumAgent" // Custom agent type for Cambium
		}
	} else {
		if s.Container {
			s.InsightAgentType = "ContainerCustom"
		} else {
			s.InsightAgentType = "CambiumAgent"
		}
	}
...
func (s *Service) SendMetricsBulk(metrics []models.MetricData) error {
	...
	for i, metric := range metrics {
		...
		// Add all metric data points for this device
