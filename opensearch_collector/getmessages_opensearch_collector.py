...
def process_get_data(log_queue, cli_config_vars, if_config_vars, agent_config_vars, messages, worker_process,
    ...
    logger.info('Starting get data from OpenSearch on collector {} ...'.format(collector_id))
    ...
    if not es_conn and collector_id == 0:
        config_error(logger, 'OpenSearch connection error')
        # # send close single for each worker
        for i in range(0, worker_process):
            messages.put(CLOSED_MESSAGE)
        messages.close()
        logger.error("Failed to get data from OpenSearch caused by connection issue.")
        time.sleep(1)
        return False
    ...
    if agent_config_vars['his_time_range']:
        ...
        for timestamp in range(agent_config_vars['his_time_range'][0],
            ...
            logger.debug('Getting data from OpenSearch with query:' + str(query_body))

            # build query with chunk
            query_messages_elasticsearch(logger, cli_config_vars, if_config_vars, agent_config_vars, es_conn,
                                         query_body, messages, start_time * 1000, agent_config_vars['indeces'])
        ...
        query_messages_elasticsearch(logger, cli_config_vars, if_config_vars, agent_config_vars, es_conn, query_body,
                                     messages, start_time * 1000, agent_config_vars['indeces'])
    ...
    logger.info('Finish getting data from OpenSearch')
...
def get_es_connection(logger, agent_config_vars):
    ...
    # Handle authentication (OpenSearch client format)
    if 'http_auth' in agent_config_vars['elasticsearch_kwargs']:
        auth_value = agent_config_vars['elasticsearch_kwargs']['http_auth']
        connection_params['http_auth'] = tuple(auth_value.split(':', 1))
    
    # Handle SSL/TLS parameters for OpenSearch
    ...
    # Handle use_ssl parameter for OpenSearch
    ...
    logger.debug(f"OpenSearch connection params: {connection_params}")
    
    try:
        os_client = OpenSearch(hosts, **connection_params)
        logger.info("Successfully created OpenSearch client connection")
        return os_client
    except Exception as e:
        logger.error('Could not contact OpenSearch with provided configuration.')
...
def build_es_connection_hosts(logger, agent_config_vars):
    ...
    hosts = []
    for uri in agent_config_vars['os_uris']:
        host = {}
...
def query_messages_elasticsearch(logger, cli_config_vars, if_config_vars, agent_config_vars, es_conn, query_body,
                                 messages, sampling_timestamp, index):
    is_metric = 'METRIC' in if_config_vars['project_type']
    logger.debug('Starting query server es')

    # get total number of messages using scroll API for OpenSearch compatibility
    try:
        response = es_conn.search(index=index, body=query_body, scroll='2m', ignore_unavailable=False)
    except Exception as e:
    ...
    # Use scroll API for pagination (OpenSearch compatible)
    ...
    try:
        ...
        logger.debug('Failed to clear scroll context: {}'.format(str(e)))
...
def get_agent_config_vars(logger, config_ini):
    ...
    with open(config_ini) as fp:
        ...
        # Check for opensearch section first (new), fallback to elasticsearch (legacy)
        config_section = 'opensearch' if config_parser.has_section('opensearch') else 'elasticsearch'
        
        try:
            # opensearch/elasticsearch settings
            elasticsearch_config = {
                'port': config_parser.get(config_section, 'port'),
                'http_auth': config_parser.get(config_section, 'http_auth'),
                'use_ssl': config_parser.get(config_section, 'use_ssl'),
                'ssl_version': config_parser.get(config_section, 'ssl_version'),
                'ssl_assert_hostname': config_parser.get(config_section, 'ssl_assert_hostname'),
                'ssl_assert_fingerprint': config_parser.get(config_section, 'ssl_assert_fingerprint'),
                'verify_certs': config_parser.get(config_section, 'verify_certs'),
                'ca_certs': config_parser.get(config_section, 'ca_certs'),
                'client_cert': config_parser.get(config_section, 'client_cert'),
                'client_key': config_parser.get(config_section, 'client_key')
            }
            ...
            os_uris = config_parser.get(config_section, 'os_uris')
            query_json_str = config_parser.get(config_section, 'query_json')
            query_json_file = config_parser.get(config_section, 'query_json_file')
            query_chunk_size = config_parser.get(config_section, 'query_chunk_size')
            indeces = config_parser.get(config_section, 'indeces')
            query_time_offset_seconds = config_parser.get(config_section, 'query_time_offset_seconds', fallback=0)

            # time range
            his_time_range = config_parser.get(config_section, 'his_time_range')

            # proxies
            agent_http_proxy = config_parser.get(config_section, 'agent_http_proxy')
            agent_https_proxy = config_parser.get(config_section, 'agent_https_proxy')

            # logs
            project_field = config_parser.get(config_section, 'project_field')
            project_whitelist = config_parser.get(config_section, 'project_whitelist')

            # message parsing
            document_root_field = None
            try:
                document_root_field = config_parser.get(config_section, 'document_root_field', raw=True)
            except configparser.NoOptionError:
                pass

            component_field = config_parser.get(config_section, 'component_field', raw=True)
            default_component_name = config_parser.get(config_section, 'default_component_name', raw=True)
            instance_field = config_parser.get(config_section, 'instance_field', raw=True)
            instance_field_regex = config_parser.get(config_section, 'instance_field_regex', raw=True)
            instance_whitelist = config_parser.get(config_section, 'instance_whitelist')
            default_instance_name = config_parser.get(config_section, 'default_instance_name', raw=True)
            device_field = config_parser.get(config_section, 'device_field', raw=True)
            device_field_regex = config_parser.get(config_section, 'device_field_regex', raw=True)
            timestamp_field = config_parser.get(config_section, 'timestamp_field', raw=True) or '@timestamp'
            target_timestamp_timezone = config_parser.get(config_section, 'target_timestamp_timezone',
                                                          raw=True) or 'UTC'
            timestamp_format = config_parser.get(config_section, 'timestamp_format', raw=True)
            timezone = config_parser.get(config_section, 'timezone') or 'UTC'
            data_fields = config_parser.get(config_section, 'data_fields', raw=True)
            aggregation_data_fields = config_parser.get(config_section, 'aggregation_data_fields', raw=True)
            safe_instance_fields = config_parser.get(config_section, 'safe_instance_fields', fallback='')
        ...
        # uris required
        if len(os_uris) != 0:
            os_uris = [x.strip() for x in os_uris.split(',') if x.strip()]
        else:
            return config_error(logger, 'os_uris')
