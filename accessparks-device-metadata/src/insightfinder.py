...
def _build_session() -> requests.Session:
    ...
    retry = Retry(
        total=3,
        connect=3,
        # read=0 (not retried): a read timeout means the request may already
        # have reached the server and been processed — since these POSTs are
        # not idempotent (they append log/metric data), retrying here risks
        # double-ingesting data whose outcome we never actually saw. connect
        # failures are safe to retry (nothing was sent); status failures are
        # safe-ish (the server gave us a definite response, however bad).
        read=0,
        status=3,
        backoff_factor=1.0,
        backoff_jitter=0.3,
