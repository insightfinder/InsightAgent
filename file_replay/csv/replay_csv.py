        ...
        try:
            ...
            with open(replay_file) as csv_file:
                for line in csv_file:
                    csv_data.append(delim.split(line.rstrip()))
            # sort by timestamp
...
def get_agent_config_vars():
    ...
    if os.path.exists(os.path.abspath(os.path.join(__file__, os.pardir, 'config.ini'))):
        ...
        all_fields = {'filters': filters,
                      'required_fields': required_fields,
                      'optional_fields': optional_fields,
                      'data_fields': data_fields}
        all_fields = check_csv_field_indeces(csv_field_names, all_fields)
        csv_vars = dict()
        for filter_type in all_fields['filters']:
            if 'filter' in all_fields['filters'][filter_type]:
                csv_vars[filter_type] = all_fields['filters'][filter_type]['filter']
            else:
                csv_vars[filter_type] = ''
        for optional_field in all_fields['optional_fields']:
            if 'index' in all_fields['optional_fields'][optional_field]:
                csv_vars[optional_field] = all_fields['optional_fields'][optional_field]['index']
            else:
                csv_vars[optional_field] = ''
        for required_field in all_fields['required_fields']:
            if 'index' in all_fields['required_fields'][required_field]:
                csv_vars[required_field] = all_fields['required_fields'][required_field]['index']
            else:
                csv_vars[required_field] = ''
        csv_vars['data_fields'] = all_fields['data_fields']
        ...
        # add csv variables to config_vars
        config_vars.update(csv_vars)
...
def check_csv_field_indeces(csv_field_names, all_fields):
    # required
    for field in all_fields['required_fields']:
        all_fields['required_fields'][field]['index'] = get_field_index(csv_field_names, all_fields['required_fields'][field]['name'], field, True)
    ...
    for field in all_fields['filters']:
        if len(all_fields['filters'][field]['name']) != 0:
            ...
            all_fields['filters'][field]['filter'] = str(filters_temp)
