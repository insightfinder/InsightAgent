...
func (s *Service) SendLogData(logEntries []LogData) error {
	...
	return s.SendLogDataRaw(logDataList)
}

// SendLogDataRaw sends raw log data as maps (preserves IP addresses and other fields)
func (s *Service) SendLogDataRaw(logs []map[string]interface{}) error {
	if len(logs) == 0 {
		return nil
	}
	...
	logrus.Debugln("projectName:", s.ProjectName, "instanceName:", hostname)
