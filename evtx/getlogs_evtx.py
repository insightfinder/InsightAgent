...
def get_agent_config_vars():
    ...
    if os.path.exists(os.path.abspath(os.path.join(__file__, os.pardir, 'config.ini'))):
        ...
        if len(file_path) != 0:
            ...
            if len(files) > 1:
                files = [ i for i in                                    # for each generated sublist \
                    j for j in map(lambda k:                            #   for each item in replay_sa_files \ 
                        ...
                        files)                                          # \
                    if os.path.basename(j).split('.')[-1] == 'evtx' ]   # keep only evtx files
            else:
                files = files[0]
                files = get_file_list_for_directory(files) if files[-1] == '/' else files
