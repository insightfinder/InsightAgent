...
function usage()
{
	echo "Usage: ./deployment/newrelic_install.sh -i PROJECT_NAME -u USER_NAME -k LICENSE_KEY -s SAMPLING_INTERVAL_MINUTE -r REPORTING_INTERVAL_MINUTE -t AGENT_TYPE -p API_KEY
...
if [ -z "$API_KEY" ] || [ -z "$AGENT_TYPE" ] || [ -z "$REPORTING_INTERVAL" ] || [ -z "$SAMPLING_INTERVAL" ] || [ -z "$USERNAME" ] || [ -z "$PROJECTNAME" ] || [ -z "$LICENSEKEY" ]; then
	usage
	exit 1
fi
...
if [ -z "$INSIGHTAGENTDIR" ]; then
 ...
 #       echo "Wrong home directory. Run ./proxy_deployment/install.sh from InsightAgent-master folder"
 #       exit 1
#fi
