...
AGENT_TYPE = proc or cadvisor or docker_remote_api or cgroup or metricFileReplay or logFileReplay or daemonset or hypervisor or elasticsearch or collectd or ec2monitoring or jolokia or nfdump or kvm or kafka or elasticsearch-storage or elasticsearch-log or opentsdb or zabbix. Reporting/Sampling interval supports integer value denoting minutes and 10s i.e 10 seconds as a valid value"
}
...
if [ $AGENT_TYPE == 'daemonset' ]; then
	...
	COMMAND_REPORTING="$PYTHONPATH $INSIGHTAGENTDIR/$AGENT_TYPE/getmetrics_zabbix.py -d $INSIGHTAGENTDIR -w $SERVER_URL -c $CHUNK_SIZE 2>$INSIGHTAGENTDIR/log/reporting.err 1>$INSIGHTAGENTDIR/log/reporting.out"
	if [ "$IS_SECOND_REPORTING" = true ] ; then
		createCronSeconds "${COMMAND_REPORTING}" $TEMPCRON
	else
		createCronMinute $REPORTING_INTERVAL "${COMMAND_REPORTING}" $TEMPCRON
