...
sudo pip install --no-index --find-links='pip_packages' sdcclient requests
```
...
```
./deployment/install.sh -i PROJECT_NAME -u USER_NAME -k LICENSE_KEY -s SAMPLING_INTERVAL_MINUTE -t sysdig -w SERVER_URL
```
