#!/usr/bin/env python3
"""
TEST MODE: Orchestrator script to process first X Zabbix maps and send to InsightFinder.
...
# TEST CONFIGURATION - Load from config.py
MAX_MAPS_TO_PROCESS = config.max_maps_to_process
...
def main():
    ...
    for map_idx, map_data in enumerate(maps, 1):
        ...
        maps_with_data_processed += 1
        logger.info(f"  Processing map {maps_with_data_processed}/{MAX_MAPS_TO_PROCESS} with data...")
